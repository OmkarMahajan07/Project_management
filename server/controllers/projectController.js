    ...
    try { 
        ...
        if(!workspace.members.some((member) => member.userId === userId && member.role === 'ADMIN')) {
            ...
            if (!project) {
                ...
                return res.status(403).json({ message: "Access denied. Admins or Team Lead only." });
            }
        }
...
export const addMember = async (req, res) => {
    try {
        ...
        if (project.team_lead !== userId) {
            return res.status(403).json({ message: "Access denied. Only Team Lead can add members." });
        }
