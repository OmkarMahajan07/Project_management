    ...
    if(new Date(task.due_date).toLocaleDateString() !== new Date().toDateString()){ 
      ...
      await step.run('check-if-task-is-completed', async () => {
        ...
        if (task.status !== "DONE") {
          ...
          await sendEmail({
            to: task.assignee.email,
            subject: `Reminder: Pending Task - ${task.project.name}`,
            body: `<div style="max-width: 600px;">
                  <h2>Hi ${task.assignee.name}, </h2>
                  ...
                  "><strong>Description :< /strong> ${task.
                description}</p>
                  <p style="margin: 6px 0;"><strong>Due
                  Date :< /strong> ${new Date(task.due_date).
                toLocaleDateString()}</p>
                  </div>

                  <a href="${origin}" style="background-color:
                  I #007bff; padding: 12px 24px; border-radius:
